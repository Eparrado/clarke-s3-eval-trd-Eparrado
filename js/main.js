...
function addMoreReason() {
  ...
  .then(function(json){
    ...
    var oldReasonContainer = '';
    var newReasonContainer = '';

    for (var i = 0; i < newReason.length; i++) {
      oldReasonContainer += '<div class="row margin-bottom-m"><h5 class="highlight-content center no-margin margin-tablet">' + newReason[i].title + '</h5><p class="center no-margin">' + newReason[i].description + '</p></div>';
      newReasonContainer = gridContainer.innerHTML + oldReasonContainer;
    }
